...
def combine_spectra(spectrum1, spectrum2):
    ...
    wl_out = np.arange(min(wl_spectrum1.min(), wl_spectrum2.min()), 
    ...
    tbdata = Table([wl_out, flux_combine, error_combine], names=['wavelength', 'flux', 'error'])
...
def write_final_spectrum(header, tbdata, fname):
    ...
    #        hdr[kwd] += hdr2[kwd]
    tbdata.meta = header
    tbdata.write('{name}_combine.fits'.format(name=fname), 
                 overwrite=True)
    tbdata.write('{name}_combine.ascii'.format(name=fname), 
                 format='ascii.commented_header', overwrite=True)
    tbdata.write('{name}_combine.dat'.format(name=fname), 
...
def median_n_exposures(spectra, Nbins):
    ...
    for i in range(len(spectra)):
        ...
        y = spectra[i]['flux']
        stat_med, bin_edges, binnumber = binned_statistic(x,y, statistic='median', 
                                                           bins=Nbins)
    ...
    if len(spectra) == 1:
        ...
        stat_std, bin_edges, binnumber = binned_statistic(x,y, statistic='std', 
                                                           bins=Nbins)
        y_med = stat_med
        y_std = stat_std
        all_xs = xs
    ...
    result = {'flux_all': all_ys, 'wavelength_all':all_xs, 
             'flux_med_per_bin':y_med, 'flux_std_per_bin':y_std,
             'bins':bin_centers}
...
def get_data_to_dict_NEW(ztf_dr_path, ztf_n=-99 ,
                         sdss_name_pattern ='J204303',
                        ztf_all_bands = []):
    ...
    if ztf_n > 0 :
        ...
        if len(matched_name)<1:
            print(f'Could not find a ZTF light curve in {ztf_dr_path} that\ contains{sdss_name_pattern}')
        else:
    ...
    # read the ZTF data
    ztf_lc = Table.read(os.path.join(ztf_dr_path,
                                     ztf_fname), format='csv')
